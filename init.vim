...
"--- ...but sometimes we need the backtick, so map two backticks (type fast) to the backtick
inoremap `` `
...
"----- Use <leader>c to trigger completion (when in insert mode)
inoremap <silent><expr> <leader>c coc#refresh()

"----- Use <leader>c to toggle completion (normal mode)
...
"----- GoTo code navigation: others rarely work: gr coc-references, gt coc-type-definition, gi coc-implementation
