...
local theme = lush(function(injected_functions)
  ...
  return {
    ...
    Search         { fg=c.c08, bg=c.c03 },      -- Last search pattern highlighting (see 'hlsearch'). Also used for similar items that need to stand out.
    IncSearch      { Search },                  -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    CurSearch      { Search },                  -- Highlighting a search pattern under the cursor (see 'hlsearch')
    Substitute     { Search },                  -- |:substitute| replacement text highlighting

    TabLineFill    { fg=c.c03, bg=c.c_4 },      -- Tab pages line, where there are no labels
    TabLine        { TabLineFill },             -- Tab pages line, not active tab page label
    TabLineSel     { fg=c.c_5, bg=c.c05, gui="bold" },       -- Tab pages line, active tab page label
    ...
    Trans1 { fg=c.c05, bg=c.c01},
    SLine2 { fg=c.c05, bg=c.c01, gui="bold" },
    Trans2 { fg=c.c01, bg=c.c_3},
    SLine3 { fg=c.c03, bg=c.c_4 },
    Trans3 { fg=c.c05, bg=c.c_3},
    ...
    Visual         { fg=c.c06, bg=c.c_1 },                -- Visual mode selection
    ...
    Cursor         { bg=c.c10, fg=c.blk },      -- Character under the cursor
    ...
    CursorColumn   { bg=c.c_4 },                -- Screen-column at the cursor, when 'cursorcolumn' is set.
    TermCursor     { Cursor },                  -- Cursor in a focused terminal
    TermCursorNC   { bg=c.c00, fg=c.c03 },      -- Cursor in an unfocused terminal

    LineNr         { fg=c.c01 },                -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    ...
    Pmenu          { fg=c.c04, bg=c.c_2},       -- Popup menu: Selected item.
    PmenuSel       { fg=c.c10, bg=c.c02 },      -- Popup menu: Normal item.
    PmenuKind      { Pmenu },                   -- Popup menu: Normal item "kind"
    PmenuKindSel   { PmenuSel},                 -- Popup menu: Selected item "kind"
    PmenuExtra     { Pmenu},                    -- Popup menu: Normal item "extra text"
    PmenuExtraSel  { PmenuSel},                 -- Popup menu: Selected item "extra text"
    PmenuSbar      { fg=c.c10, bg=c.c_3 },      -- Popup menu: Scrollbar.
    PmenuThumb     { fg=c.wht, bg=c.c_1},       -- Popup menu: Thumb of the scrollbar.
    ...
    DiffAdd        { bg=c.grb,   fg=c.c08 },    -- Diff mode: Added line |diff.txt|
    DiffChange     { bg=c.org,   fg=c.blk},     -- Diff mode: Changed line |diff.txt|
    DiffText       { DiffChange, fg=c.c08},     -- Diff mode: Changed text within a changed line |diff.txt|
    DiffDelete     { bg=c.ppr,   fg=c.c08 },    -- Diff mode: Deleted line |diff.txt|
    ...
    Type           { fg=c.grb },           -- (*) int, long, char, etc.
